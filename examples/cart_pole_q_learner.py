...
#Must run "set PYTHONPATH=c:/modelicagym/modelicagym" first to be able to have the setup.py file be cited to run the commands
...
def cart_pole_train_qlearning(cart_pole_env, max_number_of_steps=500, n_episodes=4, visualize=True):
    ...
    for episode in range(n_episodes):
        print("Check E", episode, "-1")
        x, x_dot, phi, phi_dot = cart_pole_env.reset()
        print("Check E", episode, "0")
...
def run_ql_experiments(n_experiments=100,
                       n_episodes=10,
                       visualize=False,
                       m_cart=10,
                       m_pole=1,
    ...
    """
    #path = "C:/modelicagym/modelicagym/resources/dymola/windows/ModelicaGym_CartPole_CS_3.fmu"
    curr_dir = os.path.dirname(os.path.abspath(__file__));

    path = os.path.join(curr_dir, 'ModelicaGym_CartPole_CS.fmu')
    print("Path: ", path)
    config = {
        'm_cart': m_cart,
        'm_pole': m_pole,
    ...
    env_name = "DymolaCSCartPoleEnv-v0"

    register(
        id=env_name,
        entry_point='examples:DymolaCSCartPoleEnv',
        kwargs=config
    )
    trained_agent_s = []
    episodes_length_s = []
...
if __name__ == "__main__":
    _, episodes_lengths, exec_times = run_ql_experiments(visualize=True, log_level=logging.DEBUG)
